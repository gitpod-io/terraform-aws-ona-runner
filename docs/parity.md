...
be private. Ordinary HTTP forwarding and HTTPS CONNECT support nonstandard
ports; credential injection remains limited to authorized HTTPS origins on 443.
Configured internal service hostnames are excluded from forwarding on all ports.
The proxy protects its own endpoint and lookup hostname as well as the runner
service aliases passed by the module. Other aliases and literal addresses are
not discovered by hostname matching. The configured customer egress proxy
retains its connection port and bypass settings. Environments receive public
inspection trust through runner bootstrap; signing keys remain outside them.

The proxy runs in its own Fargate service. Its application and execution roles
remain separate from the runner roles. It can read its server key, public client
CA and binding lookup token from one Secrets Manager resource. The runner alone
can initialize that resource and access the separate client-signing authority.
Secret versions are generated by the runner and are absent from Terraform state.

Ordinary task restarts preserve the issuer and serving material. Deliberate issuer
replacement requires matching proxy material, restarted tasks, and fresh bootstrap
identities for recreated Environments and warm pools. Resuming a retained VM does
not migrate an identity signed by another issuer.

Environments connect directly to private task addresses through Cloud Map on port
8443 and authenticate with their runner-issued client certificate. The module
reuses its private runner namespace and service registration, including with
`restrict_ingress`. Only the proxy security group can reach the runner's binding
lookup on port 7072. The lookup also requires a shared bearer token generated by
the runner and loaded by both processes from the existing proxy material record.
It uses private HTTP, retaining the private-network confidentiality assumption.
Health and metrics use port 9092, reachable from the existing
ECS security group. When enabled, the existing broad load-balancer ingress rule
excludes port 7072. No additional load balancer is created.

The existing runner updater also discovers the credential proxy and updates its
application and CA-init images before replacing the runner. The first transition
from a runtime without binding authentication requires a coordinated manual
runner-first upgrade with automatic updates disabled: wait for all old runner
tasks to drain before updating the credential proxy and applying matching module
configuration. Old proxies fail lookups during this transition; missing token
material prevents new proxies from starting. The ordinary proxy-first updater
cannot perform that initial transition. Roll back the proxy before restoring an
unauthenticated runner.

Token values remain outside Terraform state. The runner upgrades existing material
without changing certificates. Token replacement is independent of CA replacement:
stop the credential proxy, replace only the `lookupToken` JSON field with an
unpadded base64url encoding of 32 cryptographically random bytes, restart all
runner tasks, then restart the proxy. Retain all other fields and keep the value
out of task arguments, environment variables and diagnostic output. Automatic
rotation/reload is not provided; this coordinated replacement interrupts lookups.

The proxy receives `proxy_config` and the existing custom CA bundle through its
CA initialization container. Configuration changes roll the task deployment.
Environment-to-proxy and internal binding requests use direct private connections;
upstream requests obey customer proxy and bypass settings. URL-based trust bundles
must remain reachable during task initialization, following the existing runner
