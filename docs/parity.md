...
initialize that resource and access the separate client-signing authority. Secret
versions are generated by the runner and are absent from Terraform state.
...
lookup on port 7072. Health and metrics use port 9092, reachable from the existing
...
application and CA-init images before replacing the runner. Its application role
and execution role remain separate from the runner roles.
